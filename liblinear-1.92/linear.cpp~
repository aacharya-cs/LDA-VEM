...
double* Solver_MCSVM_CS::Solve(double *w)
{
	...
	double *alpha_return = new double[l*nr_class]; // alpha to be returned to the calling program
	int *index = new int[l];
	...
	{
		v += alpha[i];
                alpha_return[i] = alpha[i]; // assigning final values of dual variable alpha
		if(fabs(alpha[i]) > 0)
			nSV++;
	}
	for(i=0;i<l;i++)
...
model* train(const problem *prob, const parameter *param)
{
	...
	model_->bias = prob->bias;
        model_->nr_trpts = l;      // number of training points
	...
	else
	{
		...
		if(param->solver_type == MCSVM_CS)
		{
			...
			model_->alpha = Solver.Solve(model_->w);
		}
		else
		{
			if(nr_class == 2)
...
int save_model(const char *model_file_name, const struct model *model_)
{
	...
	for(i=0; i<w_size; i++)
	{
		int j;
		for(j=0; j<nr_w; j++)
			fprintf(fp1, "%.16g ", model_->w[i*nr_w+j]);
		fprintf(fp1, "\n");
	}
        fclose(fp1);

        FILE *fp2 = fopen("alphaval.txt","w");
        if (ferror(fp2) != 0)
          printf("error in opening file");
        else
        {  
	 for(i=0; i<model_->nr_trpts; i++)
	 {
		int j;
		for(j=0; j<nr_w; j++)
			fprintf(fp2, "%.16g ", model_->alpha[i*nr_w+j]);
		fprintf(fp2, "\n");
	 } 
         fclose(fp2);
        }  
