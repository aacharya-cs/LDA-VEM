...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    double *tmp2, *tmp4, *tmp7, *option;
    int N, T, K2, nK; // number of instances
    int dimx, dimy;
    
    tmp1   = mxGetField(prhs[0],0,"zeta");
    N      = mxGetNumberOfElements (tmp1);
    K2     = (int)mxGetScalar(mxGetField(prhs[0],0,"K2"));
    T      = (int)mxGetScalar(mxGetField(prhs[0],0,"T"));
    option = (double*)mxGetPr(prhs[2]); // option=1, NPDSLDA; (int)option[0]==0, NPLDA
